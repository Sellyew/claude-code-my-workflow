...
controls. Ninety-five cases, and it finishes in seconds.
...
4. SYNC TO MAIN (when ready)
   ...
   git status --porcelain  # must be empty — git-guardrails denies a merge into a
                           # dirty tree, and "kept working on main" is the whole
                           # point of a worktree, so expect it NOT to be empty
...
# ...or let git do the stashing, which the hook exempts (tracked changes only):
...
Clear the tree first — stash with a label, merge, pop — or use `--autostash`, which the hook
exempts because git establishes the clean precondition itself rather than the hook guessing that
some earlier segment of the command line did. After a `--autostash` squash the merge result is
*staged* and your restored in-flight edits are *unstaged*, so the plain `git commit` above records
the merge and leaves your work alone. The one thing `--autostash` does not cover is untracked
files: if the branch adds a file at a path you are holding untracked, git aborts with *"untracked
working tree files would be overwritten"* after the autostash has already been taken. When
porcelain shows `??` lines you care about, use the explicit `-u` stash.
