...
maintainers wrote the bound before running anything: over the preregistered holdout of 300
spike-in samples of known concentration, the nominal 95% intervals must cover the true
concentration at **94% or better**. The campaign runs. Measured coverage: **267 of 300 = 89.0%**,
with a standard error of 1.8 percentage points — not a near miss, and not noise.
...
bound it missed, verbatim, next to the disposition. The failing campaign is kept in the
repository — not deleted, and not re-run, because a second draw against unchanged inputs is the
draw that ends up published. Two downstream functions inherited the default; both are listed,
one affected, one explicitly not and why.

What made this a real bound was not its value. It was that missing it had a consequence written
down in advance.

A threshold you are still willing to move after seeing the number was never a threshold, and a
project only finds out which kind it had by missing one.
:::
...
| **Bypass** | `bypassPermissions` | Skips **all** permission prompts and safety checks — including writes to protected paths (`.git`, `.vscode`, `.idea`, `.husky`, `.claude`). Anthropic's docs advise reserving it for isolated environments | CI/CD, headless scripts, and — eyes open — trusted-repo daily driving: nothing prompts, so pair it with plan-first and the shipped `git-guardrails` hook |
...
What it still cannot see is written in its docstring rather than implied away: a `git merge` carried inside `bash -c`, an interpreter, an alias, a script, or a Makefile; the op hidden inside a command substitution; a `cd` into a different repository; and anything that dirties the tree between the check and the run. **Root of trust** exists because the settings file and the hook directory are the layer everything else stands on: a shell one-liner can disable every guard in the repo in a single redirect, and nothing in the transcript looks unusual afterwards. Reads pass, and `Edit` / `Write` stay allowed so a change to a gate arrives as a reviewable diff rather than an invisible overwrite; the hatch is `ALLOW_ROOT_OF_TRUST_WRITE=1`.

Know what that second one is and is not. It reads the command line as text, so it catches the direct forms — redirects, `tee`, `cp`/`mv`/`rm`, in-place edits — and the payload carriers it knows about (`bash -c`, `env -S`), including behind common wrappers. It cannot see inside an interpreter it does not read (`python3 -c`, `perl -e`), a path piped into a deleter, or an execution form nobody has enumerated yet; the hook's own docstring lists that residual rather than implying it does not exist. **It is defense-in-depth against the silent path, not a proof that the gates cannot be disabled** — the real backstops are that `Edit`/`Write` changes are reviewable and that you chose the bypass posture deliberately. A guard whose advertised coverage exceeds its actual coverage is worse than none, because you stop looking.
...
## One character, and a hook that was never there
...
Nothing breaks. There is no error, because a hook whose command cannot be found simply does not
run; the session continues, tools are allowed, and the transcript reads exactly as it did the week
before. The guard is in the settings file, in the README, and in the onboarding doc. It is not in
the process. Weeks later a calibration file is overwritten by an agent that the hook had been
written to stop.

Gate 9, ledger coverage, is the check that would have caught it: it opens every hook registered in
`.claude/settings.json` and requires the file to exist, be invocable, and be tracked in git. A
mistyped path fails the build the moment it is committed.
...
controls. Sixty-seven cases, and it finishes in seconds.

So check that a guard still objects, not only that it still fires: a control dies as easily by
running and no longer objecting as by never running at all, and proof of wiring is not proof of
effect.
:::
...
between catching it and teaching it for a semester — which is the thing that tracing a number to
its source cannot buy you, since a number is verified only once every place it appears has been
enumerated and compared, and the copy nobody regenerates is the one an audience reads.
:::
...
(v2.6, path-scoped to agent and skill definitions) states the rule as a question to ask before
...
A team qualifying its reproduction agent runs a positive control: reproduce a pinned coefficient
from a published analysis and report it. The agent returns `-0.07314` in nine seconds. Correct to
the last printed digit. The team is briefly delighted.
...
Skip that fence and the cost is a control that reads green for the wrong reason: an agent that can
reach the answer will reach the answer, so independence has to be built into where the reviewer
stands, not into what it is told.
:::
...
specifications, or findings. Its default is **EXCLUDE**: a candidate is out unless a written
criterion admits it, with the evidence recorded per candidate. It ends in an adjudication table
...
Nothing malfunctioned. "Relevant" was never defined, so each agent supplied its own definition,
and any paper can be connected to any topic by a sufficiently generous reading. INCLUDE was the
path of least resistance and there was no criterion pushing the other way.
...
The moment each verdict has to quote the sentence that earns a paper its place, a delegated
opinion becomes a delegated verdict — because now the agent pays for inclusion instead of for
exclusion.
:::
...
| Review Fencing | `review-fencing.md` | `.claude/agents/**/*.md`, `.claude/skills/**/SKILL.md` — reviewer independence as a property of the *environment*: neutral copy outside the checkout, prior verdicts withheld, own reading before prior findings, positive controls fenced from committed answer keys (v2.6) |
...
| Root-of-trust guard | PreToolUse (command) | `.claude/hooks/root-of-trust-guard.py` — denies shell writes to `.claude/settings*.json`, `.claude/hooks/`, `.githooks/`; reads and `Edit`/`Write` still pass (`ALLOW_ROOT_OF_TRUST_WRITE=1`) |
