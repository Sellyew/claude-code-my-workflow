...
There is a further lesson in how these two hooks were hardened, and it is the one most likely to transfer to whatever you are guarding. Every review round from the guards' first hardening onward found at least one *new* class of spelling that reached the same file — among them a comment that merely mentions a heredoc, a shell glob, a path folded through a symlink, a redirection operator missing from the list, ANSI-C `$'…'` quoting that survives unquoting as a stray `$`, an uppercase command word that a case-insensitive filesystem resolves to the same binary, and a quote sitting inside the protected directory's own name. Every fix was correct, and every fix was followed by another class. Read that as a measurement rather than a run of bad luck: **a textual recognizer over an adversarial spelling space does not converge**, and "the last round found nothing" is then evidence about the round, not about the guard. The practical consequence is not to give up on the guard — it still costs an accident a denial — but to change what you measure. Convergence is *new defect classes ceasing to appear*, not *the current test set passing*; and until you can claim the first, the honest thing a docstring can say is where it has been probed, not where it is complete. If you need a guarantee rather than a speed bump, the mechanism has to stop being a text scan: a real `deny` rule in the permission system, or a wrapper the operation cannot go around.

There is a boundary inside that lesson worth separating from it, because the two get conflated. Every class listed above is a **textual** transformation the shell performs on the command line — the information is present in the string, so a scanner can be taught to see it, and "keep looking" is the right response. A symbolic link is not a spelling. It is filesystem state, and no amount of reading the command line recovers it: a link named anything at all, pointing at the directory you protect, is invisible to a text scan while the shell writes straight through it. That class is disclosed in the guard rather than closed, and the distinction matters when you build your own — it tells you which gaps are worth another round of parsing and which are telling you the mechanism is the wrong shape for what you are trying to guarantee.
...
controls. 224 cases, and it finishes in seconds.
:::
