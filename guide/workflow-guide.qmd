...
to check it again. **They reported everything was fine.** Recall: 0 out of 20.
...
**All six gates stayed green.** The count patterns are deliberately compound — they require
...
Three references carry it, loaded on demand rather than living in every session's context:
...
```bash
./scripts/backtest.sh    # eight gates: consistency, currency, links, spec, staleness, hygiene, derived counts
```
...
In a controlled study, 150 autonomous agents were given the same data and the same questions.
...
Claude Code ships with built-in skills beyond this template's 61: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
Alias resolution is **provider-dependent**, and point versions move faster than any guide can. The single source of truth is [`model-versions.md`](https://github.com/pedrohcgs/claude-code-my-workflow/blob/main/.claude/references/model-versions.md), which carries the current versions, the per-provider alias table, and a `verified_on` date with an expiry — `scripts/check-model-versions.sh` fails the build when it goes stale. **Do not hard-code a point version here.**

**Retirement notice:** Sonnet 4 and the original Opus 4 retire on **2026-06-15**. If your environment pins one of these (`ANTHROPIC_MODEL` env, hard-coded model IDs in CI, etc.), migrate before then. See [TROUBLESHOOTING.md](https://github.com/pedrohcgs/claude-code-my-workflow/blob/main/TROUBLESHOOTING.md) for the migration checklist.
:::
...
- **Label each fork `estimand` or `estimate`.** In staggered DiD, comparison group, base
  period, and aggregation change *what is being estimated* — averaging over them is
  meaningless.
...
- **Then attack the identifying assumption** with a named, computable statistic — E-value,
  Cinelli–Hazlett RV, Oster δ, Rosenbaum Γ, Rambachan–Roth honest DiD. Never a naive pre-test.
...
The guide includes 61 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
...
| `/vaccinate` | Grade the grader: seed known defects + a clean control into a copy of a real artifact, run the check or review agent, report **recall** and **false-positive rate** into `quality_reports/qualification/LEDGER.md` | Before trusting any gate or referee simulation for a decision that matters |
...
| `/challenge` | `.claude/skills/challenge/` | Stress-test a finding against the choices you didn't make — specification curve over the discrete forks, then named sensitivity statistics (E-value, Cinelli–Hazlett RV, Oster δ, honest DiD) against the identifying assumption |
