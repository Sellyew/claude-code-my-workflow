...
Considered for v1.10.0 and pulled before ship: a `.claude-plugin/marketplace.json` exposing `review-suite` / `simulation-suite` as installable subsets. **UNBLOCKED 2026-08-21:** [#11278](https://github.com/anthropics/claude-code/issues/11278) **closed 2025-11-09** — the path-resolution bug this deferral rested on is fixed. The à-la-carte packaging decision is now a scope call, not a technical blocker. Re-evaluate against the v2.5 plan. **Re-affirmed as a documented non-goal in v2.0** (see CHANGELOG v2.0.0 "Notes"); the template stays fork-the-whole-thing until #11278 resolves.
